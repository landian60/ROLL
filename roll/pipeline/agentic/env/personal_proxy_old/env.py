...
class PersonalProxyEnv(Env):
    ...
    def _init_user_llm(self):
        ...
        try:
            ...
            base_url = self.user_llm_config.get("base_url", "https://dashscope.aliyuncs.com/compatible-mode/v1")
            ...
            logger.info(f"用户LLM初始化成功: base_url={base_url}, model={self.user_model_name}")
    ...
    def _init_judge_llm(self):
        ...
        try:
            ...
            base_url = self.judge_llm_config.get("base_url", "https://dashscope.aliyuncs.com/compatible-mode/v1")
            ...
            logger.info(f"打分LLM初始化成功: base_url={base_url}, model={self.judge_model_name}")
        except Exception as e:
            logger.warning(f"打分LLM初始化失败: {e}")
    
    def _generate_user_persona(self) -> str:
        """随机生成用户拟人系统提示词"""
        self.current_user_persona = random.choice(self.user_system_prompts)
...
只返回意图，不要其他内容。"""
    ...
    def _retrieve_intent_from_history(self, context: str, person: str) -> Optional[IntentHistory]:
        """
        从意图判断历史中检索相似情景的意图
        ...
        for history in self.intent_history:
            score = 0.0
            # 检查情景相似度
            if context_lower in history.context.lower() or history.context.lower() in context_lower:
                score += 0.5
            # 检查人物相似度
            if person_lower in history.person.lower() or history.person.lower() in person_lower:
                score += 0.3
    ...
    def _generate_personal_description(self, intent: str, intent_reason: str, personal_memory: str) -> str:
        """
...
基于以上信息，这是一个关于用户在当前情景下的个性化描述。"""
    ...
    def _judge_personal_description(self, personal_description: str, context: str, person: str) -> Tuple[float, str]:
        """
        使用qwen api对个人描述进行打分
...
理由：[详细说明]"""
        ...
        try:
            response = self.judge_client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "你是一个专业的评估专家，负责评估个人描述的质量。"},
                    {"role": "user", "content": prompt}
                ],
                temperature=0.3,
                max_tokens=300
...
"""
    ...
    def reset(self, seed: Optional[int] = None):
        ...
        try:
            with all_seed(seed):
                ...
                self.current_context = random.choice(contexts)
                self.current_person = random.choice(persons)
                
                # 生成用户拟人
                self._generate_user_persona()
                
                # 模拟用户反应和意图
                user_response, user_intent = self._simulate_user_response(self.current_context, self.current_person)
                
                # 从历史中检索相似意图
                history_match = self._retrieve_intent_from_history(self.current_context, self.current_person)
                
                if history_match:
                    # 使用历史记录中的意图（如果用户有反馈，使用反馈）
                    self.current_intent = history_match.feedback or history_match.intent
                    self.current_intent_reason = history_match.feedback_reason or history_match.intent_reason
                else:
                    # 使用新生成的意图
                ...
                # 检索个人记忆
                self.current_personal_memory = self._retrieve_personal_memory(self.current_intent)
    ...
    def step(self, action: str):
        ...
        # 保存意图判断历史（如果这是新的）
        if not self._retrieve_intent_from_history(self.current_context, self.current_person):
    ...
    def parse_action(self, text: str):
        """解析动作"""
        return default_parser_action_func(text, self.action_pattern, {}, self.special_token_list)
    
    def provide_user_feedback(self, intent: Optional[str] = None, intent_reason: Optional[str] = None, 
